	...
	for _, tt := range tests {
		t.Run(tt.contentType, func(t *testing.T) {
			if got := imageTypeFromContentType(tt.contentType); got != tt.expected {
				t.Errorf(
					"imageTypeFromContentType(%q) = %v, want %v",
					tt.contentType,
					got,
					tt.expected,
				)
	...
	for _, tt := range tests {
		t.Run(tt.extension, func(t *testing.T) {
			if got := ImageTypeFromExtension(tt.extension); got != tt.expected {
				t.Errorf(
					"ImageTypeFromExtension(%s) = %v, want %v",
					tt.extension,
					got,
					tt.expected,
				)
	...
	for _, tt := range tests {
		t.Run(tt.filename, func(t *testing.T) {
			if got := ImageTypeFromFilename(tt.filename); got != tt.expected {
				t.Errorf(
					"ImageTypeFromFilename(%s) = %v, want %v",
					tt.filename,
					got,
					tt.expected,
				)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if detected != tt.detected {
				t.Errorf(
					"ImageTypeFromMagicBytes() detected = %v, want %v",
					detected,
					tt.detected,
				)
			}
...
func TestDetectImageType(t *testing.T) {
	...
	if got != ImageTypePng {
		t.Errorf(
			"DetectImageType with PNG magic bytes = %v, want %v",
			got,
			ImageTypePng,
		)
	}
	...
	if got != ImageTypeGif {
		t.Errorf(
			"DetectImageType with fallback to extension = %v, want %v",
			got,
			ImageTypeGif,
		)
	}
	...
	if got != ImageTypePng {
		t.Errorf(
			"DetectImageType default = %v, want %v",
			got,
			ImageTypePng,
		)
	}
...
func TestPartErrors(t *testing.T) {
	...
	if ErrNilPackage.Error() != "package is nil" {
		t.Errorf(
			"ErrNilPackage.Error() = %v, want 'package is nil'",
			ErrNilPackage.Error(),
		)
	}
}
...
func TestStylesPartFixedContentType(
	t *testing.T,
) {
	sp := &StylesPart{}
...
func TestNumberingPartFixedContentType(
	t *testing.T,
) {
	np := &NumberingPart{}
...
func TestSettingsPartFixedContentType(
	t *testing.T,
) {
	sp := &SettingsPart{}
...
func TestWebSettingsPartFixedContentType(
	t *testing.T,
) {
	wsp := &WebSettingsPart{}
...
func TestHeaderPartFixedContentType(
	t *testing.T,
) {
	hp := &HeaderPart{}
...
func TestFooterPartFixedContentType(
	t *testing.T,
) {
	fp := &FooterPart{}
...
func TestFootnotesPartFixedContentType(
	t *testing.T,
) {
	fp := &FootnotesPart{}
...
func TestEndnotesPartFixedContentType(
	t *testing.T,
) {
	ep := &EndnotesPart{}
...
func TestVbaProjectPartFixedContentType(
	t *testing.T,
) {
	vp := &VbaProjectPart{}
...
func TestCommentsPartAddComment(t *testing.T) {
	...
	c := comments.AddComment(
		"Test Author",
		"Test comment",
	)
	if c.Id() != 1 {
		t.Errorf(
			"AddComment() returned id = %v, want 1",
			c.Id(),
		)
	}
	...
	if c2.Id() != 2 {
		t.Errorf(
			"AddComment() returned id = %v, want 2",
			c2.Id(),
		)
	}
}
