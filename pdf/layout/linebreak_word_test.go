...
// ===========================================================================
// MIXED SCRIPTS (LATIN + CJK) TESTS
// ===========================================================================

// TestWordBreaking_MixedScripts verifies Word-like handling of mixed Latin and CJK text.
// Word allows breaks between CJK characters and at transitions between scripts.
func TestWordBreaking_MixedScripts(t *testing.T) {
	wlb := NewWordLineBreaker()

	tests := []struct {
		name           string
		text           string
		expectBreaks   bool // Whether we expect breaks between CJK chars
		noBreakInLatin bool // Whether Latin words should stay together
	}{
		{
			name:           "latin_then_cjk",
			text:           "Hello \u4E16\u754C",
		...
		{
			name:           "interleaved",
			text:           "The \u4E16\u754C is world",
			expectBreaks:   true,
			noBreakInLatin: true,
		},
		{
			name:           "japanese_with_latin",
			text:           "\u3053\u3093\u306B\u3061\u306F Hello",
			expectBreaks:   true,
			noBreakInLatin: true,
		},
		{
			name:           "korean_with_latin",
			text:           "\uC548\uB155 world here",
			expectBreaks:   true,
			noBreakInLatin: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Check that breaks exist between CJK characters
			runes := []rune(tt.text)
			for i := 1; i < len(runes); i++ {
				prevCJK := IsCJK(runes[i-1])
				currCJK := IsCJK(runes[i])

				if prevCJK && currCJK {
					// Should be a break opportunity between CJK chars
					// (Unless kinsoku shori prohibits it)
					foundBreak := false
					for _, opp := range opportunities {
						if opp.RunePosition == i &&
					...
					// Only check if not a non-starter situation
					if !IsNoBreakBefore(
						runes[i],
					) &&
						!IsNoBreakAfter(
						...
						if tt.expectBreaks &&
							!foundBreak {
							t.Logf(
								"Note: No break between CJK chars at position %d (may be kinsoku)",
								i,
							)
						}
					}
...
// TestWordBreaking_FilePaths verifies Word-like handling of file paths.
// Word allows breaks after path separators (/ and \).
func TestWordBreaking_FilePaths(t *testing.T) {
	wlb := NewWordLineBreaker()

	tests := []struct {
		name             string
		text             string
		expectBreakAfter []string // Characters after which breaks are expected
	}{
		{
			name: "unix_path",
			text: "Path: /home/user/docs",
			expectBreakAfter: []string{
				"/home",
				"/user",
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, after := range tt.expectBreakAfter {
				idx := strings.Index(
					tt.text,
					after,
				)
				...
				breakPos := idx + len(after)

				found := false
				for _, opp := range opportunities {
					if opp.Position == breakPos &&
						opp.Type == BreakAllowed {
						found = true
				...
				if !found {
					t.Logf(
						"Expected break after %q at position %d",
						after,
						breakPos,
					)
				}
...
// TestWordBreaking_EdgeCases tests edge cases that could cause issues.
func TestWordBreaking_EdgeCases(t *testing.T) {
	wlb := NewWordLineBreaker()
