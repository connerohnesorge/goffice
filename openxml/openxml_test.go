...
func TestQualifiedName(t *testing.T) {
	...
	t.Run(
		"ParseQualifiedName with namespace",
		func(t *testing.T) {
			qn := ParseQualifiedName(
...
func TestAttribute(t *testing.T) {
	t.Run("NewAttribute", func(t *testing.T) {
		...
		if attr.LocalName() != "name" {
			t.Errorf(
				"expected local name 'name', got %s",
				attr.LocalName(),
			)
		}
		if attr.NamespaceURI() != "http://example.com" {
			t.Errorf(
				"expected namespace URI, got %s",
				attr.NamespaceURI(),
			)
		}
		if attr.Prefix() != "ex" {
			t.Errorf(
				"expected prefix 'ex', got %s",
				attr.Prefix(),
			)
		}
		if attr.Value() != "value" {
...
func TestCompositeElement(t *testing.T) {
	t.Run(
		...
		func(t *testing.T) {
			...
			if elem.LocalName() != "document" {
				t.Errorf(
					"expected local name 'document', got %s",
					elem.LocalName(),
				)
			}
	...
	t.Run("AppendChild", func(t *testing.T) {
		...
		child1 := NewCompositeElement(
			"",
			"child1",
			"",
		)
		child2 := NewCompositeElement(
			"",
			"child2",
			"",
		)
		...
		if parent.FirstChild() != Element(
			...
			t.Error(
				"expected child1 as first child",
			)
		}

		if parent.LastChild() != Element(child2) {
			t.Error(
				"expected child2 as last child",
			)
		}

		if child1.Parent() != Element(parent) {
			t.Error(
				"expected parent as child1's parent",
			)
		}
	})
	...
	t.Run("PrependChild", func(t *testing.T) {
		...
		if parent.FirstChild() != Element(
			...
			t.Error(
				"expected child2 as first child after prepend",
			)
		}
	})
	...
	t.Run("RemoveChild", func(t *testing.T) {
		...
		if parent.ChildCount() != 1 {
			t.Errorf(
				"expected 1 child after remove, got %d",
				parent.ChildCount(),
			)
		}
		...
		if child1.Parent() != nil {
			t.Error(
				"expected nil parent after remove",
			)
		}
	})
	...
	t.Run("ReplaceChild", func(t *testing.T) {
		...
		if parent.FirstChild().
			...
			"child2" {
			t.Error(
				"expected child2 as first child",
			)
		}

		if child1.Parent() != nil {
			t.Error(
				"expected nil parent for replaced child",
			)
		}
	})
...
func TestAttributes(t *testing.T) {
	...
	t.Run("RemoveAttribute", func(t *testing.T) {
		...
		removed := elem.RemoveAttribute(
			"name",
			"",
		)
		if !removed {
			t.Error(
				"expected attribute to be removed",
			)
		}
		...
		if ok {
			t.Error(
				"expected attribute to not be found",
			)
		}
	})
...
func TestXmlSerialization(t *testing.T) {
	...
	t.Run(
		"Element with children OuterXml",
		func(t *testing.T) {
			parent := NewCompositeElement(
				"",
				"parent",
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			if !strings.Contains(
				...
				t.Errorf(
					"expected attribute in XML, got %s",
					xml,
				)
			}
		},
	...
	t.Run("InnerXml", func(t *testing.T) {
		...
		child1 := NewLeafElementWithText(
			"",
			"child1",
			"",
			"text1",
		)
		child2 := NewLeafElementWithText(
			"",
			"child2",
			"",
			"text2",
		...
		if strings.Contains(
			...
			t.Error(
				"InnerXml should not contain parent element",
			)
		}
		if !strings.Contains(
			innerXml,
...
func TestCloning(t *testing.T) {
	t.Run(
		...
		func(t *testing.T) {
			...
			if parent.ChildCount() != 1 {
				t.Error(
					"original should be unchanged after modifying clone",
				)
			}
		},
	)
...
func TestHelpers(t *testing.T) {
	t.Run("First", func(t *testing.T) {
		...
		leaf1 := NewLeafElementWithText(
			"",
			"leaf",
			"",
			"first",
		)
		comp := NewCompositeElement(
			"",
			"comp",
			"",
		)
		leaf2 := NewLeafElementWithText(
			"",
			"leaf",
			"",
			"second",
	...
	t.Run("All", func(t *testing.T) {
		...
		if count != 2 {
			t.Errorf(
				"expected 2 leaf elements, got %d",
				count,
			)
		}
	})
	...
	t.Run("Descendants", func(t *testing.T) {
		...
		grandchild := NewLeafElementWithText(
			"",
			"grandchild",
			"",
			"text",
...
func TestIntegrationOpenXmlWithPackaging(
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf(
					"Document part not found: %v",
					err,
				)
			}

			// Verify content type
			ct, _ := pkg2.ContentTypes().
				GetContentType("/word/document.xml")
			...
			if !strings.Contains(
				...
				t.Error(
					"Document content not preserved",
				)
			}
		},
	)
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf(
					"Failed to save modifications: %v",
					err,
				)
			}
			oxPkg2.Close()
			...
			if err != nil {
				t.Error(
					"Styles part should exist after modification",
				)
			}
		},
	)
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			if docData, ok := docPart.(*OpenXmlPartData); ok {
				...
				if err != nil {
					t.Fatalf(
						"Failed to add child part: %v",
						err,
					)
				}

				// Verify child part is accessible
				retrieved, err := docData.GetPartById(
					"rId1",
				)
				if err != nil {
					t.Fatalf(
						"GetPartById failed: %v",
						err,
					)
				}
				if retrieved != stylesPart {
					t.Error(
						"Wrong child part returned",
					)
				}
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			docCT, err := ct.GetContentType(
				"/word/document.xml",
			)
			if err != nil ||
				docCT != ContentTypeWordprocessingMLDocument {
				t.Error(
					"Document content type not set correctly",
				)
			}

			stylesCT, err := ct.GetContentType(
				"/word/styles.xml",
			)
			if err != nil ||
				stylesCT != ContentTypeStyles {
				t.Error(
					"Styles content type not set correctly",
				)
			}
	...
	t.Run(
		"element to part serialization roundtrip",
		func(t *testing.T) {
			tmpPath := filepath.Join(
				t.TempDir(),
				"roundtrip_test.docx",
			...
			if !strings.Contains(
				...
				t.Error(
					"Content not serialized correctly",
				)
			}
			if !strings.Contains(
				string(data),
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			if count2 != len(parts) {
				t.Errorf(
					"After reload: expected %d parts, got %d",
					len(parts),
					count2,
				)
			}
...
func TestIntegrationPartRootElement(
	t *testing.T,
) {
	t.Run(
		"PartRootElement with part association",
		func(t *testing.T) {
			tmpPath := filepath.Join(
				t.TempDir(),
			...
			if root.Part() != part {
				t.Error(
					"Root element should reference its part",
				)
			}

			if part.RootElement() != root {
				t.Error(
					"Part should reference its root element",
				)
			}
		},
	)
	...
	t.Run(
		"PartRootElement Save and Reload",
		func(t *testing.T) {
			tmpPath := filepath.Join(
				t.TempDir(),
				"save_reload_test.docx",
			)
			pkg, _ := packaging.Create(tmpPath)
	...
	t.Run(
		"PartRootElement Clone does not copy part",
		func(t *testing.T) {
			root := NewPartRootElement(
				NamespaceWordprocessingML,
				"document",
			...
			if !ok {
				t.Fatal(
					"Clone should be a PartRootElementBase",
				)
			}
			...
			if cloneRoot.Part() != nil {
				t.Error(
					"Cloned root element should not be associated with a part",
				)
			}
		},
	)
...
func TestIntegrationRelationshipManagement(
	t *testing.T,
) {
	t.Run(
		"relationship ID generation across parts",
		func(t *testing.T) {
			tmpPath := filepath.Join(
				t.TempDir(),
				"rel_ids_test.docx",
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			externalRel := NewExternalRelationship(
				"rId1",
			...
			if externalRel.TargetMode() != TargetModeExternal {
				t.Error(
					"External relationship should have external target mode",
				)
			}

			if externalRel.Target() != "https://example.com" {
				t.Error(
					"Target should be the external URL",
				)
			}
		},
	)
	...
	t.Run(
		...
		func(t *testing.T) {
			// External hyperlink
			extHyperlink := NewHyperlinkRelationship(
				"rId1",
				"https://example.com",
				true,
				nil,
			...
			if !extHyperlink.IsExternal() {
				t.Error(
					"External hyperlink should be external",
				)
			}

			// Internal hyperlink (bookmark)
			intHyperlink := NewHyperlinkRelationship(
				"rId2",
				"#bookmark",
				false,
				nil,
			...
			if intHyperlink.IsExternal() {
				t.Error(
					"Internal hyperlink should not be external",
				)
			}
		},
	)
