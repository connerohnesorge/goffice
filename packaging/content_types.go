...
func (ct *ContentTypes) GetContentType(uri string) (string, error) {
	...
	ext := strings.ToLower(strings.TrimPrefix(URIExtension(uri), "."))
...
func (ct *ContentTypes) MarshalToXML() ([]byte, error) {
	...
	for ext, contentType := range ct.defaults {
		xmlCT.Defaults = append(xmlCT.Defaults, xmlDefault{
			Extension:   ext,
	...
	for uri, contentType := range ct.overrides {
		xmlCT.Overrides = append(xmlCT.Overrides, xmlOverride{
			PartName:    uri,
			ContentType: contentType,
		})
	}
