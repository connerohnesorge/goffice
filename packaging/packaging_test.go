	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := tt.cap.String(); got != tt.want {
				t.Errorf(
					"PackageCapability.String() = %v, want %v",
					got,
					tt.want,
				)
			}
...
func TestPackageCreateAndSave(t *testing.T) {
	...
	if _, err := os.Stat(path); os.IsNotExist(
		...
		t.Error(
			"Expected file to exist after save",
		)
	}
...
func TestPackageSaveAs(t *testing.T) {
	...
	if _, err := os.Stat(path2); os.IsNotExist(
		...
		t.Error(
			"Expected path2 to exist after SaveAs",
		)
	}
}
...
func TestPackageClose(t *testing.T) {
	...
	if err != ErrPackageClosed {
		t.Errorf(
			"CreatePart() on closed package error = %v, want ErrPackageClosed",
			err,
		)
	}
}
...
func TestPackageReadOnly(t *testing.T) {
	...
	if pkg2.Capability() != Read {
		t.Errorf(
			"Capability() = %v, want Read",
			pkg2.Capability(),
		)
	}

	// Write operations should fail
	_, err = pkg2.CreatePart(
		"/test.xml",
		"application/xml",
	)
	if err != ErrReadOnly {
		t.Errorf(
			"CreatePart() on read-only package error = %v, want ErrReadOnly",
			err,
		)
	}
}
...
func TestCreatePartDuplicate(t *testing.T) {
	...
	if err != ErrPartExists {
		t.Errorf(
			"Second CreatePart() error = %v, want ErrPartExists",
			err,
		)
	}
}
...
func TestCreatePartInvalidURI(t *testing.T) {
	...
	if err != ErrInvalidURI {
		t.Errorf(
			"CreatePart('') error = %v, want ErrInvalidURI",
			err,
		)
	}
}
...
func TestGetPartNotFound(t *testing.T) {
	...
	if err != ErrPartNotFound {
		t.Errorf(
			"Part() error = %v, want ErrPartNotFound",
			err,
		)
	}
}
...
func TestDeletePart(t *testing.T) {
	...
	if err != ErrPartNotFound {
		t.Errorf(
			"Part() after delete error = %v, want ErrPartNotFound",
			err,
		)
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.partURI, func(t *testing.T) {
			if got := RelationshipPartURI(tt.partURI); got != tt.want {
				t.Errorf(
					"RelationshipPartURI(%q) = %q, want %q",
					tt.partURI,
					got,
					tt.want,
				)
...
func TestContentTypesSetDefault(t *testing.T) {
	...
	if got := ct.GetDefault("png"); got != "image/png" {
		t.Errorf(
			"GetDefault(png) = %q, want image/png",
			got,
		)
	}
	...
	if got := ct.GetDefault("jpg"); got != "image/jpeg" {
		t.Errorf(
			"GetDefault(jpg) = %q, want image/jpeg",
			got,
		)
	}
}
...
func TestContentTypesOverride(t *testing.T) {
	...
	contentType, err := ct.GetContentType(
		"/word/document.xml",
	)
	if err != nil {
		t.Fatalf(
...
func TestContentTypesFallbackToDefault(
	t *testing.T,
) {
	ct := NewContentTypes()
...
func TestContentTypesNotFound(t *testing.T) {
	...
	if err != ErrContentTypeNotFound {
		t.Errorf(
			"GetContentType() error = %v, want ErrContentTypeNotFound",
			err,
		)
	}
}
...
func TestContentTypesSerialization(t *testing.T) {
	...
	if got := ct2.GetDefault("png"); got != "image/png" {
		t.Errorf(
			"After roundtrip, GetDefault(png) = %q, want image/png",
			got,
		)
	}

	contentType, _ := ct2.GetContentType(
		"/word/document.xml",
	)
	if contentType != "application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml" {
		t.Error(
...
func TestCreateRelationshipDuplicate(
	t *testing.T,
) {
	rels := NewRelationships("/")
	...
	if err != ErrRelationshipExists {
		t.Errorf(
			"Create() with duplicate ID error = %v, want ErrRelationshipExists",
			err,
		)
	}
}
...
func TestGetRelationshipNotFound(t *testing.T) {
	...
	if err != ErrRelationshipNotFound {
		t.Errorf(
			"Get() error = %v, want ErrRelationshipNotFound",
			err,
		)
	}
}
...
func TestDeleteRelationship(t *testing.T) {
	...
	if err != ErrRelationshipNotFound {
		t.Errorf(
			"Get() after delete error = %v, want ErrRelationshipNotFound",
			err,
		)
	}
}
...
func TestRelationshipsByType(t *testing.T) {
	...
	if count != 2 {
		t.Errorf(
			"ByType(type-a) count = %d, want 2",
			count,
		)
	}
}

func TestRelationshipsSerialization(
	t *testing.T,
) {
	rels := NewRelationships("/")
	_, _ = rels.Create(
	...
	if rel2.TargetMode() != TargetModeExternal {
		t.Errorf(
			"After roundtrip, rId2 TargetMode() = %v, want External",
			rel2.TargetMode(),
		)
	}
}
...
func TestCorePropertiesDates(t *testing.T) {
	...
	if cp.Created() == nil ||
		!cp.Created().Equal(now) {
		t.Error("Created() not set correctly")
	}

	if cp.Modified() == nil ||
		!cp.Modified().Equal(now) {
		t.Error("Modified() not set correctly")
	}
}

func TestCorePropertiesSerialization(
	t *testing.T,
) {
	cp := NewCoreProperties()
...
func TestPackageRelationshipsByType(
	t *testing.T,
) {
	pkg, _ := Create(
		filepath.Join(t.TempDir(), "test.docx"),
	...
	if count != 2 {
		t.Errorf(
			"RelationshipsByType() count = %d, want 2",
			count,
		)
	}
}
...
func TestPartRelationships(t *testing.T) {
	...
	rel, err := part.CreateRelationship(
		"media/image1.png",
	...
	if err != nil {
		t.Fatalf(
			"Part.CreateRelationship() error = %v",
			err,
		)
	}
	...
	if rels == nil {
		t.Fatal(
			"PartRelationships() returned nil",
		)
	}
...
func TestPackageRoundtrip(t *testing.T) {
	...
	if string(
		...
		t.Error(
			"Document part data not preserved",
		)
	}
	...
	if count != 1 {
		t.Errorf(
			"Package relationships count = %d, want 1",
			count,
		)
	}

	// Verify part relationships
	docRels := pkg2.PartRelationships(
		"/word/document.xml",
	)
	if docRels == nil || docRels.Count() != 1 {
		t.Error(
			"Part relationships not preserved",
		)
	}

	// Verify core properties
	if pkg2.CoreProperties().
		Title() !=
		"Test Document" {
		t.Errorf(
			"CoreProperties.Title() = %q, want Test Document",
			pkg2.CoreProperties().Title(),
		)
	}
}
