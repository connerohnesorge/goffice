...
FlatOPC is a single-XML representation of an OPC (Open Packaging Convention) document. Instead of a ZIP file with multiple parts, all content is embedded in one XML document. This research is shared across all document types (Word, Excel, PowerPoint).
...
|-----------|-------------|
| `pkg:name` | Part URI (e.g., `/word/document.xml`) |
...
- Important for embedded documents (e.g., embedded HTML, RTF)
