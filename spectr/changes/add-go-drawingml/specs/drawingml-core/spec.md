# DrawingML Core Types

Delta specification for core DrawingML types including shapes, fills, effects, and transforms.

## Summary

This spec defines the core DrawingML types that form the foundation of all visual elements in Office Open XML documents. These types are shared across Word, Presentation, and Spreadsheet documents.

## Motivation

DrawingML (Drawing Markup Language) provides the common graphics layer for all Office document types. The core types handle:

- Shape geometry (preset and custom shapes)
- Fill patterns (solid, gradient, pattern, image)
- Line properties and styles
- Visual effects (shadows, glow, reflection)
- 2D transforms (position, size, rotation)
- Color definitions and transformations

Without these shared types, each document SDK would need to duplicate significant code for handling visual elements.
...
drawingml/
  main/               # Core DrawingML types (a: namespace)
    transform.go      # Transform2D, Offset, Extents
    geometry.go       # PresetGeometry, CustomGeometry
    fill.go           # SolidFill, GradientFill, PatternFill, BlipFill
    line.go           # LineProperties, dash styles, joins, ends
    effect.go         # EffectList, shadows, glow, reflection
    shape.go          # ShapeProperties, ShapeStyle
    color.go          # All color types and transforms
```

### Type Naming Convention

Go types use shortened idiomatic names:
...
func PixelsToEMU(pixels int, dpi int) int64
...
```go
// Create transform with position and size
xfrm := main.NewXfrm(
    main.WithOffset(main.PointsToEMU(72), main.PointsToEMU(72)),  // 1 inch from top-left
    main.WithExtents(main.InchesToEMU(2), main.InchesToEMU(1)),   // 2" x 1"
    main.WithRotation(90),  // 90 degrees
)

// Access properties
x, y := xfrm.OffsetPt()           // in points
w, h := xfrm.ExtentsPt()          // in points
rot := xfrm.RotationDegrees()     // in degrees
flipH, flipV := xfrm.Flipped()    // booleans
```
...
geom := main.NewPrstGeom("roundRect",
    main.WithAdjust("adj", 16667),  // corner radius
)

// Access shape type
shapeType := geom.Prst()  // "roundRect"
adjustments := geom.AdjustValues()
```
...
fill := main.NewSolidFill(main.NewSrgbClr("FF0000"))

// Solid fill with scheme color and transform
fill := main.NewSolidFill(
    main.NewSchemeClr("accent1", main.WithAlpha(50000)),  // 50% alpha
)

// Gradient fill
grad := main.NewGradFill(
    main.WithLinearGradient(90),  // 90 degree angle
    main.WithGradientStop(0, main.NewSrgbClr("FFFFFF")),
    main.WithGradientStop(100000, main.NewSrgbClr("000000")),
)
```
...
```go
// Outer shadow
effects := main.NewEffectLst(
    main.WithOuterShadow(
        main.WithShadowBlur(main.PointsToEMU(4)),
        main.WithShadowDist(main.PointsToEMU(3)),
        main.WithShadowDir(45),  // degrees
        main.WithShadowColor(main.NewSrgbClr("000000", main.WithAlpha(50000))),
    ),
)
```
...
- Transform2D creation and property access
- EMU conversion accuracy
- All preset geometry types
- All fill types and color combinations
- Effect property validation
- Line property combinations
- Color transform calculations

### Integration Tests

- Shape properties round-trip (marshal/unmarshal)
- Theme color resolution
- Relationship resolution for blip fills
- Complex nested effects

### Validation Tests

- Invalid preset geometry names
- Out-of-range rotation values
