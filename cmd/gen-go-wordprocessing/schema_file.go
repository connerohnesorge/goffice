...
type SchemaFileMetadata struct {
	...
	Namespace string
	// NamespacePrefix is the derived namespace prefix (w14, w15, x14, etc.).
...
// Examples:
//   - Word 2010: "http://schemas.microsoft.com/office/word/2010/wordml" → "w14"
//   - Excel 2013: "http://schemas.microsoft.com/office/spreadsheetml/2010/11/main" → "x14"
...
// Examples: "w14" → Office2010, "x15" → Office2013, "a16" → Office2016
func prefixToVersion(
	prefix string,
) FileFormatVersion {
	// Extract numeric suffix from prefix (e.g., "w14" → "14")
	suffixPattern := regexp.MustCompile(`\d+$`)
	suffix := suffixPattern.FindString(prefix)
