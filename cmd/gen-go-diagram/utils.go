...
// It determines if a type belongs to drawingml or is local to diagram package.
func getGoPackage(ns string) string {
	switch {
	case ns == nsDrawingML:
		return drawingMLPkg
	case ns == nsDiagram:
		return "" // diagram namespace types are in local package
	case strings.Contains(ns, "diagram"):
		return "" // all diagram-related types are local
	case strings.Contains(ns, drawingMLPkg):
		return drawingMLPkg
	case strings.HasPrefix(ns, nsDiagramPrefix):
		return "" // Microsoft diagram extensions are local
	default:
		return ""
	}
}
...
func getGoNamespace(uri string) string {
	switch uri {
	case nsDiagram:
		return "openxml.NamespaceDiagram"
	case nsDrawingML:
